 ...
 * @returns {{ name: string, surname: string, lastname: string, birthday: string, startYear: string, faculty: string }} Объект студента
 */
function makeStudentFromData(data) {
    ...
    if (!student.startYear) errors.push({ field: 'startYear', message: 'Не указано начало обучения' });
 ...
 * @returns {{ id: string, name: string, surname: string, lastname: string, birthday: string, startYear: string, faculty: string }[]} Массив студентов
 */
 ...
 * @returns {{ id: string, name: string, surname: string, lastname: string, birthday: string, startYear: string, faculty: string, createdAt: string, updatedAt: string }} Объект студента
 */
 ...
 * @param {{ name?: string, surname?: string, lastname?: string, birthday?: string, startYear?: string, faculty?: string }} data - Объект с изменяемыми данными
 * @throws {ApiError} Студент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, name: string, surname: string, lastname: string, birthday: string, startYear: string, faculty: string, createdAt: string, updatedAt: string }} Объект студента
 */
...
module.exports = createServer(async(req, res) => {
    ...
    .on('listening', () => {
        if (process.env.NODE_ENV !== 'test') {
            ...
            console.log(`POST ${URI_PREFIX} - создать студента, в теле запроса нужно передать объект { name: string, surname: string, lastname: string, birthday: string, startYear: string, faculty: string}`);
            console.log(`GET ${URI_PREFIX}/{id} - получить студента по его ID`);
            console.log(`PATCH ${URI_PREFIX}/{id} - изменить студента с ID, в теле запроса нужно передать объект { name?: string, surname?: string, lastname?: string, birthday?: string, startYear?: string, faculty?: string}`);
